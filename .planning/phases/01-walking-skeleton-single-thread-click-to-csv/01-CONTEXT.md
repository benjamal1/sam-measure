...
### Resolved during build (overnight autonomous session, 2026-07-08)
- **D-09 (resolves RESEARCH.md Open Question #2 / Assumption A1):** CSV `Date` column = the day/measurement-folder date (e.g. `05-11-26` from `D12 05-11-26/`), NOT the batch-start date embedded in the batch folder name (`Batch 8 04-24-26/`). Confirmed from real data: the old flat-folder convention (`08-03-25/`, `08-04-25/` — no batch/day split at all) is what the user's pasted ImageJ ground-truth CSV rows (Date `8/1/25`) came from, and `Batch` is a separate plain integer column — so `Date` was always the single per-image date, i.e. the day date in the newer nested convention.
- **D-10 (ruler calibration span):** Ruler photos are macro/microscope shots — only ~2cm of ruler physically fits in frame (confirmed by viewing `08-03-25/ruler.JPG`: shows numbers 15/16 with fine mm tick marks between them, photographed through a circular lens/loupe). Calibration two-click span is **0cm to 0.5cm** (5 adjacent mm tick marks), not a wide 0–10cm span — user confirmed directly. The calibration script must let the user zoom/inspect closely enough to click precise adjacent mm ticks, not assume a full ruler is visible.
- **Ground truth for MEAS-03:** Only the 4 previously-pasted ImageJ rows (5.11/5.12/5.21/5.22, dated 8/1/25) exist as ground truth — matching real images found at `~/Nextcloud/threads daily imaging/08-03-25/` and `08-04-25/` (closest available dated folders, old flat naming, includes `ruler.JPG`). User confirmed this is sufficient: "just for the purposes of output fields and sanity measurements... everything else should just be calculations and clear image analysis."
- **Overnight build environment:** Building/testing happens on the Linux OptiPlex (CPU-only, no GPU) against real synced Nextcloud data (774 images) — not the user's M2 Mac. SAM2 click-to-segment cannot be interactively tested here (no human to click); code is built, unit-tested, and run as far as automatable, with a step-by-step doc left for the user's morning test-and-troubleshoot pass on the Mac (real MPS backend, real clicking).
